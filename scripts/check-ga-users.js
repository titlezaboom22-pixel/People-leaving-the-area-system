...
const ga = snap.docs.map(d => ({ id: d.id, ...d.data() }))
  .filter(u => (u.department || '').toUpperCase().includes('GA') || u.role === 'GA' || (u.id || '').startsWith('GA'));
console.log(`\n=== ผู้ใช้ที่เกี่ยวกับ GA: ${ga.length} คน ===`);
ga.forEach(u => console.log(`  ${u.id.padEnd(10)} ${(u.name || u.displayName || '').padEnd(30)} role=${u.role} type=${u.roleType} | ${u.department}`));
process.exit(0);
