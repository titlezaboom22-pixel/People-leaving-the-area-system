...
for (const w of items.slice(0, 10)) {
  console.log(`[chain: ${w.chainId?.slice(0,20) || '-'}]`);
  console.log(`  form: ${w.sourceForm} | step ${w.step}/${w.totalSteps || '?'}`);
  console.log(`  status: ${w.status} | dept: ${w.department} | targetType: ${w.targetType || '-'}`);
  console.log(`  targetUserEmail: ${w.targetUserEmail || '-'}`);
  console.log(`  requester: ${w.requesterId} | approvedBy: ${w.approvedBy || '-'}`);
  console.log('');
}
process.exit(0);
